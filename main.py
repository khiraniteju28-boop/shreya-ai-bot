...
# QR Code link (Isse ek baar check kar lena GitHub par)
...
def handle_query(call):
    ...
    # Maine 'delete_message' hata diya hai, ab kuch gayab nahi hoga
    if call.data == "get_started":
        ...
        # Direct Photo bhejne ka tarika
        try:
            bot.send_photo(chat_id, QR_URL, caption=pay_msg, parse_mode='Markdown')
        except:
            bot.send_message(chat_id, f"⚠️ QR Image nahi mil rahi, par aap is UPI par pay karein: `{UPI_ID}`\n\n{pay_msg}", parse_mode='Markdown')
...
def verify_utr(message):
    ...
    if utr in used_utrs:
        ...
        bot.send_message(chat_id=user_id, text=f"✅ *Plan Unlocked!* \nValid till: {expiry.strftime('%Y-%m-%d %H:%M')}", parse_mode='Markdown')
...
def handle_voice(message):
    ...
    if user_id in active_users and datetime.datetime.now() < active_users[user_id]:
        ...
        bot.reply_to(message, "🔒 *Plan Expired!* Kripya naya plan lein.")
