...
# Ye link tabhi kaam karega jab aapne GitHub par photo upload ki hogi
...
def welcome(message):
    ...
    bot.send_message(message.chat.id, "✨ *Mahi AI - Voice Bot*", reply_markup=markup)

@bot.callback_query_handler(func=lambda call: True)
def handle_query(call):
    chat_id = call.message.chat.id
    # MAINE SARE DELETE COMMANDS HATA DIYE HAIN
    if call.data == "get_started":
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton("By Using Shreya 🤖", callback_data="shreya"))
        bot.send_message(chat_id, "Niche click karein:", reply_markup=markup)
    
    elif call.data == "shreya":
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton("Agree ✅", callback_data="agree"))
        bot.send_message(chat_id, "Terms se sahmat hain?", reply_markup=markup)
        ...
        bot.send_message(chat_id, "Voice Style:", reply_markup=markup)
        ...
        bot.send_message(chat_id, "🎙️ *Plan select karein:*", reply_markup=markup)

    elif "buy_" in call.data:
        days = int(call.data.split("_")[1])
        pending_plans[chat_id] = days
        pay_msg = (f"💳 *Plan: {days} Din*\n\n"
                   f"1️⃣ QR Scan karein ya UPI: `{UPI_ID}`\n"
                   "2️⃣ Payment ke baad *12-digit UTR* yahan likhein.")
        # Agar photo nahi mil rahi toh bot text bhej dega
        try:
            bot.send_photo(chat_id, QR_URL, caption=pay_msg)
        except:
            bot.send_message(chat_id, f"⚠️ Photo nahi mili, par aap pay kar sakte hain.\n\n{pay_msg}")
...
def verify_utr(message):
    ...
    if user_id in pending_plans:
        ...
        bot.send_message(user_id, f"✅ *Success!* Plan active tak: {expiry.strftime('%Y-%m-%d')}")
    else:
        bot.send_message(user_id, "❌ Pehle button se plan select karein.")
